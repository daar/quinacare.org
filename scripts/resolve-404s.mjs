...
// writing src/data/missRedirects.mjs. Targets use the native (localized)
...
for (const k of Object.keys(routeRedirects)) resolved.add(norm(k));
for (const k of [
...
// Static prefixes of PR #27's dynamic ([slug]/[year]) redirects, so concrete
// paths they already cover are treated as resolved.
const wildPrefixes = Object.keys(routeRedirects)
  .filter((k) => k.includes("["))
  .map((k) => k.split("[")[0]);
...
const paths = [...new Set(rows.filter((r) => !r.is_bot).map((r) => String(r.path)))];
...
for (const raw of paths) {
  ...
  const hit =
    index.find((it) => it.lang === lang && (it.slug.toLowerCase() === cand || it.id.toLowerCase() === cand)) ||
    index.find((it) => it.slug.toLowerCase() === cand || it.id.toLowerCase() === cand);
...
fs.writeFileSync(path.join(ROOT, "src/data/missRedirects.mjs"), out);

console.log(`404 paths (human): ${paths.length}`);
