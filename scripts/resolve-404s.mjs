...
// One entry per line, and `prettier-ignore` so nothing rewraps it.
//
// JSON.stringify writes each pair on its own line; Prettier would wrap
// the longer ones across two. That disagreement made every run produce
// a diff full of lines that only moved, hiding the handful of real key
// changes underneath. Pinning the format keeps `git diff` semantic: one
// line per redirect, so an added or changed route is the only thing
// that ever shows up.
const out =
  "// AUTO-GENERATED by scripts/resolve-404s.mjs from the Turso 404 log.\n" +
  "// Old/missed URLs -> native targets.\n" +
  "//\n" +
  "// One redirect per line and prettier-ignore below: the formatting is\n" +
  "// fixed so a diff only ever shows routes that actually changed.\n" +
  "// prettier-ignore\n" +
  "export default " +
